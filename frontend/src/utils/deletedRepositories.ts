...
function isDeletedRepositoriesPayload(value: unknown): value is DeletedRepositoriesPayload {
...
async function fetchDeletedRepositories(baseUrl: string): Promise<Set<string>> {
  if (deletedRepositoriesCache.has(baseUrl)) {
    return deletedRepositoriesCache.get(baseUrl)!;
  }
  ...
  const data = isDeletedRepositoriesPayload(rawData) ? rawData : { repositories: [] };
...
export async function filterDeletedRepositories<T extends { repository: PluginRepository }>(
  items: T[],
  baseUrl: string
): Promise<T[]> {
