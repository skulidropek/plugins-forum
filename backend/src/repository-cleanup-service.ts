...
export class RepositoryCleanupService {
	...
	public constructor(
		...
		const outputDir = path.resolve(
			config.outputDir ?? path.join(process.cwd(), "output"),
		);
		const inputDir = path.resolve(
			config.inputDir ?? path.join(process.cwd(), "input"),
		);

		this.paths = {
			oxidePlugins: path.join(outputDir, "oxide_plugins.json"),
			crawledPlugins: path.join(outputDir, "crawled_plugins.json"),
			authorDiscovered: path.join(
				outputDir,
			...
			deletedRepositoriesReport: path.join(
				outputDir,
				"deleted_repositories.json",
			),
		...
		this.concurrencyLimit = Math.max(
			1,
			config.concurrencyLimit ?? DEFAULT_CONCURRENCY,
		);
	...
	public async run(): Promise<RepositoryCleanupReport> {
		...
		this.log(
			"indexer_state",
			`Starting verification of ${repositories.length} repositories.`,
		);
			...
			return {
				...
				updatedFiles:
					missingSorted.length > 0
						? [this.paths.deletedRepositoriesReport]
						: [],
		...
		return {
			scannedRepositories:
				processedBefore + (state.nextIndex - processedBefore),
	...
	private async processRepositories(
		state: CleanupState,
	): Promise<{ rateLimitHit: boolean }> {
		for (
			let index = state.nextIndex;
			index < state.repositories.length;
			index += 1
		) {
			...
			if (this.isRateLimitResult(result)) {
				...
				this.log(
					"indexer_state",
					`Rate limit hit after processing ${state.nextIndex} repositories.`,
				);
	...
	private async loadDatasets(): Promise<DatasetBundle> {
		return {
			...
			authorDiscovered: {
				...
				data: await this.readJson<AuthorDiscoveredRepositories>(
					this.paths.authorDiscovered,
				),
			},
			authorFinderState: {
				path: this.paths.authorFinderState,
				data: await this.readJson<AuthorFinderState>(
					this.paths.authorFinderState,
				),
			},
			crawlerState: {
	...
	private collectRepositoryUsage(
		datasets: DatasetBundle,
	): Map<GitHubRepositoryName, RepositoryUsage> {
		...
		if (response.status === 403) {
			...
			if (htmlProbe === "exists") {
				return {
					...
					httpStatus: response.status,
					message:
						bodyText || "Repository accessible via HTML despite API 403.",
				};
			}
	...
	private async checkHtmlEndpoint(
		repo: GitHubRepositoryName,
		...
		try {
			...
			this.log(
				"indexer_state",
				`${repo} html probe failed: ${(error as Error).message}`,
			);
			return "unknown";
		}
	}
	...
	private async writeDeletedReport(
		missingRepos: GitHubRepositoryName[],
	): Promise<void> {
		const payload = {
...
async function runCleanupCli(): Promise<void> {
	...
	const config: RepositoryCleanupConfig = {
		...
		...(process.env.CLEANUP_DELAY_MS
			? {
					interRequestDelayMs: Number.parseInt(
						process.env.CLEANUP_DELAY_MS,
						10,
					),
				}
	...
	if (report.missingRepositories.length > 0) {
		...
		console.log(
			`Missing preview (${preview.length}/${report.missingRepositories.length}): ${preview.join(", ")}`,
		);
	} else {
